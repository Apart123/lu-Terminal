...
onMounted(() => {
  ...
  if (welcomeTexts?.length > 0) {
    ...
    terminal.writeTextOutput(
      `Welcome to Lu-terminal, coolest browser index for everyone!`
    );
    terminal.writeTextOutput(
      `Author <a href="https://github.com/Apart123" target="_blank">coder_luying</a>` +
...
.lu-terminal :deep(.ant-collapse) {
  background: none;
}

.lu-terminal :deep(.ant-collapse-borderless > .ant-collapse-item) {
  border: none;
}

.lu-terminal :deep(.ant-collapse-content > .ant-collapse-content-box) {
  padding: 0;
}
